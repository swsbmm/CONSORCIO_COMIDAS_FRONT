...
export function shoppingReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case ADD_TO_CART_MENU: {
      ...
      let priceItem = state.products.find(
        (e) => e.id === action.payload1
      ).value;
    ...
    case ADD_TO_CART_PIZZA: {
      let newItem = {
        id: action.payload1 + action.payload3,
        dataID: action.payload1,
        size: action.payload3,
      };
      let quantityItem = action.payload2;
      ...
      let priceItem =
        size === "XL"
          ...
          : size === "individual"
          ? state.products.find((e) => e.id === action.payload1).value
              .individual
          : null;
